...
class Events(object):
    ...
    def player_events(self, camera):
        #movement
        if keyboard.key_pressed("left") and keyboard.key_pressed("right") == False and self.player.colliding["left"] == False and camera.cam_velocity == 0:
            self.player.velocity["x"] = -globals.X_VELOCITY_PLAYER
        elif keyboard.key_pressed("right") and keyboard.key_pressed("left") == False and self.player.colliding["right"] == False and camera.cam_velocity == 0:
            self.player.velocity["x"] = globals.X_VELOCITY_PLAYER
        else:
