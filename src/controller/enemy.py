    ...
    if len(enemy_mtx) == 0:
        ...
        while lin < 17:
            for col in range(len(line)):
                if line[col] == "1":
                    enemy = Entity(
                        window, "./src/assets/actors/enemies/enemy.png", 1)
                    enemy.set_position(
                        col * (gvar.WIDTH/22) + (gvar.WIDTH/22)/2 - enemy.animation.width/2, lin * (gvar.HEIGHT/17) + (gvar.HEIGHT/17)/2 - enemy.animation.height)
